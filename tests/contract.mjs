...
assert.equal(object.draggable, false, "one block must be independently lockable");
assert.equal(object.element.getAttribute("data-block-draggable"), "false", "a locked block must expose its inactive drag state");
assert.equal(object.element.children[0].children[0].tabIndex, -1, "an individually locked block must leave the keyboard tab order");
