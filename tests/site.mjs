...
assert.match(manualHtml, /shell\.mjs\?v=0\.1\.27[\s\S]*manual\.mjs\?v=0\.1\.37/, "the manual must load the compact, variant, color and chance lessons without stale module caches");
...
assert.match(siteDemos["docs/shell.mjs"], /fetch\(new URL\("\.\/content\.json\?v=0\.3\.14", import\.meta\.url\)\)/, "the docs shell must load the cache-busted canonical JSON file once");
